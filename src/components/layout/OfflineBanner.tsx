  ...
  return (
    <div
      ...
      className={`w-full px-4 py-2 text-xs font-semibold flex flex-wrap items-center justify-between gap-2 shadow-sm z-40 ${
        deadCount > 0 ? 'bg-expense text-expense-fg' : 'bg-warning text-warning-fg'
      }`}
    >
      ...
      {isOnline && pendingCount > 0 && (
        <button
          onClick={handleSync}
          disabled={isSyncing}
          className="bg-white/20 hover:bg-white/30 px-2.5 py-1 rounded-lg font-bold transition-all text-xs"
        >
